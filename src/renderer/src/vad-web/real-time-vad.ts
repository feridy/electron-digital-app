...
interface RealTimeVADCallbacks {
   ...
   */
  onVADMisfire: (frame: Float32Array) => any;

  /** Callback to run when speech start is detected */
  onSpeechStart: (frame: Float32Array) => any;
...
export class AudioNodeVAD {
  static async new(ctx: AudioContext, options: Partial<RealTimeVADOptions> = {}) {
    const fullOptions: RealTimeVADOptions = {
      ...defaultRealTimeVADOptions,
      ...options
    };
    ...
    if ((fullOptions as any).ortConfig !== undefined) {
      (fullOptions as any).ortConfig(ort);
    }
    ...
    try {
      ...
      console.error(
        `Encountered an error while loading worklet. Please make sure the worklet vad.bundle.min.js included with vad-web is available at the specified path:
    ...
    const frameProcessor = new FrameProcessor(model.process, model.reset_state, {
      frameSamples: (fullOptions as any).frameSamples,
      positiveSpeechThreshold: (fullOptions as any).positiveSpeechThreshold,
      negativeSpeechThreshold: (fullOptions as any).negativeSpeechThreshold,
      redemptionFrames: (fullOptions as any).redemptionFrames,
      preSpeechPadFrames: (fullOptions as any).preSpeechPadFrames,
      minSpeechFrames: (fullOptions as any).minSpeechFrames,
      submitUserSpeechOnPause: (fullOptions as any).submitUserSpeechOnPause
    });
  ...
  pause = () => {
    ...
    this.handleFrameProcessorEvent(ev, new Float32Array());
  };
  ...
  handleFrameProcessorEvent = (
    ...
    switch (ev.msg) {
      case Message.SpeechStart:
        this.options.onSpeechStart(frame);
        break;

      case Message.VADMisfire:
        this.options.onVADMisfire(frame);
        break;
